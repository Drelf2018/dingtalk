...
// Link 链接类型消息
type Link struct {
	Title      string `json:"title"`            // 链接消息标题
	Text       string `json:"text"`             // 链接消息的内容
...
type Markdown struct {
	Title string `json:"title"` // 消息会话列表中展示的标题，非消息体的标题
	Text  string `json:"text"`  // markdown 类型消息的文本内容
}
...
type ActionCardBtn struct {
	Title     string `json:"title"`     // 按钮上显示的文本
	ActionURL string `json:"actionURL"` // 按钮跳转的 URL
}

// ActionCard actionCard 类型消息
type ActionCard struct {
	Title          string          `json:"title"`                    // 消息会话列表中展示的标题，非消息体的标题
	Text           string          `json:"text"`                     // actionCard 类型消息的正文内容，支持 markdown 语法
...
type FeedCardLink struct {
	Title      string `json:"title"`      // 每条内容的标题
	MessageURL string `json:"messageURL"` // 每条内容上午跳转链接
	PicURL     string `json:"picURL"`     // 每条内容的图片 URL ，建议使用上传媒体文件接口获取
}

// FeedCard feedCard 类型消息
type FeedCard struct {
	Links []FeedCardLink `json:"links"` // feedCard 类型消息的内容列表
}
